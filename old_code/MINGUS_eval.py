...
if __name__ == '__main__':
    ...
    modelPitch_loaded = Transformer(embedding_size, src_vocab_size, trg_vocab_size, 
                        src_pad_idx, num_heads, num_encoder_layers,
                        num_decoder_layers, forward_expansion, dropout, 
                        max_len, device).to(device)
    
    # Import model
    savePATHpitch = 'modelsPitch/MINGUSpitch_10epochs.pt'
    ...
    modelDuration_loaded = Transformer(embedding_size, src_vocab_size, trg_vocab_size, 
                        src_pad_idx, num_heads, num_encoder_layers,
                        num_decoder_layers, forward_expansion, dropout, 
                        max_len, device).to(device)
    
    # Import model
    savePATHduration = 'modelsDuration/MINGUSduration10epochs.pt'
    ...
    def generate(model, melody4gen, dict_to_ix, next_notes=10):
        ...
        # prepare input to the model
        original_seq = batch4gen(np.array(melody4gen_list), len(melody4gen_list), dict_to_ix)
        with torch.no_grad():
            for i in range(next_notes):
                
                # prepare input to the model
                melody4gen_batch = batch4gen(np.array(melody4gen_list), len(melody4gen_list), dict_to_ix)
                #trg_tensor = torch.LongTensor(outputs).unsqueeze(1).to(device)
                output = model(original_seq, melody4gen_batch)
                print(output.argmax(2)[-1, -1].shape)

                best_guess = output.argmax(2)[-1, -1].item()
                
                melody4gen_list.append(getNote(best_guess, dict_to_ix))
        return melody4gen_list
